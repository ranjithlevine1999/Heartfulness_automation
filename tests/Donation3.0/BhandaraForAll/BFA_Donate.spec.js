...
try{
test('Bandara for All -> Donation', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Sign In' }).click();
   ...
   await page.getByTestId('Bhandara for All').getByRole('button', { name: 'DONATE NOW icon' }).click();

  //await page.getByTestId('Bhandara for All').getByRole('button', { name: 'DONATE icon' }).click();
  await page.getByTestId('amount-input').click();
  await page.getByTestId('amount-input').fill('56');
  ...
  await page.getByTestId('id-number-input').fill('KKJHY5637G');
