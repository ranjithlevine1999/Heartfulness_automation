...
test('AATI Donation', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Sign In' }).click();
  await page.getByTestId('Adopt A Tree Initiative').getByRole('button', { name: 'DONATE NOW icon' }).click();
  await page.getByTestId('amount-input').click();
  
  await page.getByTestId('amount-input').fill('156');
  ...
  await page.getByTestId('donor-address').fill('615');

  await page.getByTestId('donor-postal-code').click();

  await page.getByTestId('donor-postal-code').fill('688956');
