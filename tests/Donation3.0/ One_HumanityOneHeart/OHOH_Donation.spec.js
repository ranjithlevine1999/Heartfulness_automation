...
test('test', async ({ page }) => {
  ...
  await page.getByTestId('Heartfulness - One Humanity. One Heart.').getByRole('button', { name: 'DONATE NOW icon' }).click();
  await page.getByTestId('amount-input').click();

  await page.getByTestId('amount-input').fill('568');
  ...
  await page.getByTestId('id-number-input').fill('HHTRU6748H');
