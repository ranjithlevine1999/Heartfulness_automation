...
const testData = {
  login: {
    email: "preceptor.15@mailinator.com",
    password: "password"
  },
  search: {
    name: "rakesh"
...
test('Allocating Bed --> Attending Event "Search by Name"', async ({ page }) => {
  try {
    ...
    await page.getByLabel('Email *').fill(testData.login.email);
    await page.getByLabel('Password', { exact: true }).fill(testData.login.password);
    ...
    await page.getByRole('textbox').fill(testData.search.name);
...
await page.locator('svg').nth(3).click();
  ...
  await page.getByRole('textbox').click();
  await page.getByRole('textbox').fill('bonagiri.rakesh@volunteer.heartfulness.org');
...
try{
  ...
  await page.getByRole('textbox').fill('+918998828998');
  await page.getByLabel('Search').click();
...
await page.getByRole('button', { name: ' Allocate Bed' }).click();
 ...
 // await page.getByText('Abhyasi ID').click();
  ...
  await page.getByRole('textbox').click();
  await page.getByRole('textbox').fill('INSDAA209');
  await page.getByLabel('Search').click();
...
await page.getByRole('button', { name: ' Allocate Bed' }).click();
 ...
 // await page.getByText('Abhyasi ID').click();
  await page.getByText('Abhyasi ID', { exact: true }).click();
  await page.getByRole('textbox').click();
  await page.getByRole('textbox').fill('PR-IKQS-PZPT');
  await page.getByLabel('Search').click();
//   await page.getByRole('row', { name: ' Ramar - +919898989867 5-9 F' }).getByRole('checkbox').check();
//   await page.getByRole('row', { name: ' Ramar - +919898989867 5-9 F' }).locator('i').click();
//   await page.getByLabel('Close').click();
